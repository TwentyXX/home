...
function updateBox() {
    indicators.forEach((indi, i)=>window.setTimeout(()=>{
            updateSign(i);
            indi.animate([
                {
                    width: "100vw"
            ...
            indicators[(i + screenCount - 1) % screenCount].classList.remove("newest");
 ...
 */ function updateSign(i) {
    elesList[i].forEach((spn, j)=>{
        ...
        const char = KEY_CHARS[3 * Math.floor(Math.random() * KEY_CHARS.length / 3) + j % 3];
